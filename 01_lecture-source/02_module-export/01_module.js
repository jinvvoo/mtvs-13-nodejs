 ...
 * 브러우저 상에서 동작하는 js는 script 태그를 이용하여 여러 개의 script 파일을 하나로 합쳐 사용한다.
 * 이 때 js 파일들은 동일한 유효범위(scope)를 가지게 되어 전역변수 중복과 같은 문제가 발생할 수 있었다.
 * 
 * Node.js를 이용해 js를 client side에 국한된 것이 아닌 server sice에서도 사용하고자 할 때,
 * 위의 모듈 문제는 반드시 해결해야 하는 ㅎ핵심 과제였고,
 ...
 *       종류                      ESM                            CommonJS
 * ------------------------------------------------------------------------------------
 *     재정시기                ES6에서 제정됨               ESM 이전의 대표적인 비표준
 *       문법                 import / export              require / module.exports
 *    Node.js 지원                  Y                                Y
 *   트랜스파일링                  필요                             불필요
 * ------------------------------------------------------------------------------------
 * 
 * 참고링크 : https://d2.naver.com/helloworld/12864
 * 
 */
